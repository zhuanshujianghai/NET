...
{
    public class NoticeNoViewController : BaseController
    {
        /// <summary>
        /// 公告类型列表
        /// </summary>
        ...
        public ActionResult QueryNoticetypeList(DataTableParameter param)
        {
            ...
            var lstdt = nn.QueryNoticetypeList(me.companyid);
        ...
        public ActionResult QueryNoticeList(DataTableParameter param)
        {
            ...
            var lstdt = nn.QueryNoticeListByTypeidCompanyid(typeid,me.companyid).Where(a=>a.status!=-1).ToList();
            int total = lstdt.Count;
            lstdt = lstdt.Take(param.iDisplayStart + param.iDisplayLength).Skip(param.iDisplayStart).ToList();
            return Json(new
            {
        ...
        public ActionResult EditNotice()
        {
            ...
            var me = getme();
            noticemanage_notice nn = new noticemanage_notice();
            var type = nn.QueryNoticetype(typeid,me.companyid);
            if (type == null)
            {
                rr.status = false;
                rr.msg = "未找到该公告类型";
        ...
        public ActionResult QueryNoticeCommentList()
        {
            ...
            var lstdt = nn.QueryNoticeCommentListByIdCompanyid(noticeid, me.companyid).Where(a => a.status != -1).ToList();
            int total = lstdt.Count;
